...
let DiaAcompanhamentoPageRoutingModule = class DiaAcompanhamentoPageRoutingModule {
};
...
let DiaAcompanhamentoPageModule = class DiaAcompanhamentoPageModule {
};
...
let DiaAcompanhamentoPage = class DiaAcompanhamentoPage {
    constructor(router, formBuilder, navCtrl, toast, popover, storageService, geolocation) {
        this.router = router;
                ...
                for (let dado of item.listaDadoItemGrupoConfigFormulario) {
                    if (dado.tipoDado != 'LIVRE ESCOLHA') {
                        if (dado.tipoDado === 'TEXTO LIVRE' && !this.validaCamposTextoLivre(dado)) {
                            ...
                            let acompanhamentoDado = new src_app_model_AcompanhamentoMedicoDado_model__WEBPACK_IMPORTED_MODULE_9__["AcompanhamentoMedicoDado"]();
                        ...
                        let listaItens = [];
                        let listaItensExcluir = [];
                        let acompanhamentoDado = new src_app_model_AcompanhamentoMedicoDado_model__WEBPACK_IMPORTED_MODULE_9__["AcompanhamentoMedicoDado"]();
                        acompanhamentoDado.dadoItemGrupoConfigFormulario = dado;
                        if (dado.listaItens != null && dado.listaItens.length > 0) {
                            dado.listaItens.forEach((item, indice) => {
                                let acompanhamentoItem = new src_app_model_AcompanhamentoMedicoItem_model__WEBPACK_IMPORTED_MODULE_12__["AcompanhamentoMedicoItem"]();
                        ...
                        if (dado.listaItensExcluir != null && dado.listaItensExcluir.length > 0) {
                            dado.listaItensExcluir.forEach((itemExcluir, indice) => {
                                let acompanhamentoItemExcluir = new src_app_model_AcompanhamentoMedicoItem_model__WEBPACK_IMPORTED_MODULE_12__["AcompanhamentoMedicoItem"]();
    ...
    validaCamposObrigatorios(grupo) {
        for (let item of grupo.listaItemGrupoConfigFormulario) {
            for (let dado of item.listaDadoItemGrupoConfigFormulario) {
                if (dado.flgObrigatorio) {
                    let acompanhamentoMedicoDado;
...
DiaAcompanhamentoPage.ctorParameters = () => [
...
//# sourceMappingURL=pages-acompanhamento-dia-acompanhamento-dia-acompanhamento-module-es2015.js.map